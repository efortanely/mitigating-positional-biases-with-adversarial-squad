...
# Removes 1000 original examples from SQuAD dataset
def remove_non_adversarial_qa(dataset, do_train=False, do_eval=False):
    if do_train and "train" in dataset:
        dataset["train"] = dataset["train"].filter(lambda ex: "-" in ex["id"])
    if do_eval and "validation" in dataset:
        dataset["validation"] = dataset["validation"].filter(lambda ex: "-" in ex["id"])

    return dataset

def transform_context_qa_helper(ex, adversarial_sent, shuffle_adversarial, nlp):
    context = ex["context"]

    if context is None:
        return ex
    
    # If an adversarial sentence is at the end of the context passage,
    # remove it before replacing coreferences
    if adversarial_sent:
        sentences = sent_tokenize(context)
        adversarial_sent = sentences[-1].strip()
        context = context.replace(adversarial_sent, "").strip()
        # print("ADVERSARIAL SENTENCE")
        # print(adversarial_sent)

    # print("CONTEXT")
    # print(context)

    doc = nlp(context)
    coref_replaced = doc._.coref_resolved

    if not adversarial_sent:
        ex["context"] = coref_replaced.strip()
        # print("UPDATED CONTEXT")
        # print(ex["context"])

        return ex

    if adversarial_sent and shuffle_adversarial:
        sentences = [sent.string.strip() for sent in nlp(coref_replaced).sents]
        n = len(sentences)

        if n > 1:
            insert_idx = random.randint(1, n)
        else:
            insert_idx = 1

        sentences = sentences[:insert_idx] + [adversarial_sent] + sentences[insert_idx:]
        updated_context = " ".join(sentences)
    elif adversarial_sent:
        updated_context = " ".join([coref_replaced.strip(), adversarial_sent])
    
    ex["context"] = updated_context
    # print("UPDATED CONTEXT")
    # print(ex["context"])

    return ex

# Replace coreferences and shuffle adversarial sentence into [1, n] in context
def transform_context_qa(dataset, adversarial_sent=False, shuffle_adversarial=False, do_train=False, do_eval=False):
    nlp = spacy.load("en_core_web_lg")
    neuralcoref.add_to_pipe(nlp)

    if do_train and "train" in dataset:
        dataset["train"] = dataset["train"].map(lambda ex: transform_context_qa_helper(ex, adversarial_sent, shuffle_adversarial, nlp), 
                                                          batched=False)
    if do_eval and "validation" in dataset:
        dataset["validation"] = dataset["validation"].map(lambda ex: transform_context_qa_helper(ex, adversarial_sent, shuffle_adversarial, nlp), 
                                                          batched=False)
